...
require 'SuperPayGateway.php';

class SuperPayGatewayProcessor
{
  public $superpayGateway;
  ...
  function __construct($dados_internos,$formulario, $gateway = 'SuperPayGateway')
  {
    $this->dados_internos = $dados_internos;
    $this->formulario = $formulario;

    $this->superpayGateway = new $gateway;
  ...
  public function preparePayload(){
    ...
    $this->superpayGateway->payment()->url_retorno = $this->getDadosInternos('url_retorno');
    ...
    $this->superpayGateway->payment()->capturar= $this->getDadosInternos('capturar_automaticamente');
    ...
    //   )
    $this->superpayGateway->payment()->pedido = array(
    ...
    //  )
    $this->superpayGateway->payment()->pagamento = array(
    ...
    if(is_array($this->getDadosInternos('parcelas_permitidas')))
      if(in_array($this->getFormulario('parcelas'),$this->getDadosInternos('parcelas_permitidas')))
        $this->superpayGateway->payment()->pagamento['parcelas'] = $this->getFormulario('parcelas');
      else
    ...
    if(is_array($this->getDadosInternos('operacoes_permitidas')))
      if(in_array($this->getFormulario('tipo_operacao'),$this->getDadosInternos('operacoes_permitidas')))
        $this->superpayGateway->payment()->pagamento['tipo_operacao'] = $this->getFormulario('tipo_operacao');
      else
    ...
    $this->superpayGateway->payment()->comprador = array(
